...
func NewWAL(dataDir string) (WAL, error) {
	if err := os.MkdirAll(dataDir, 0755); err != nil {
		return nil, fmt.Errorf("wal: failed to create data directory: %v", err)
	}
	filepath := filepath.Join(dataDir, constants.WALFilename)
	file, err := os.OpenFile(filepath, os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
	if err != nil {
		return nil, fmt.Errorf("wal: failed to open WAL file: %v", err)
	}
	return &wal{
		filepath: filepath,
		file:     file,
...
func (w *wal) AppendLog(entry LogEntry) error {
	...
	if _, err := w.file.Write(lengthBytes); err != nil {
		return fmt.Errorf("wal: failed to write length of the entry to WAL: %w", err)
	}
	if _, err := w.file.Write(data); err != nil {
		return fmt.Errorf("wal: failed to write data to WAL: %w", err)
	}
	return w.file.Sync()
}
...
func (w *wal) Replay(insertHandler func(key types.Key, value types.Value) error, deleteHandler func(key types.Key) error) error {
	...
	for {
		...
		if _, err := io.ReadFull(file, lengthBytes); err != nil {
			...
			return fmt.Errorf("wal: failed to read length of log entry: %w", err)
		}
		length := binary.BigEndian.Uint32(lengthBytes)
		data := make([]byte, length)
		if _, err := io.ReadFull(file, data); err != nil {
			return fmt.Errorf("wal: failed to read log entry data: %w", err)
		}
		var entry LogEntry
...
func (w *wal) Clear() error {
	...
	if err := os.Remove(w.filepath); err != nil && !os.IsNotExist(err) {
		return fmt.Errorf("wal: failed to remove WAL file: %w", err)
	}
	file, err := os.OpenFile(w.filepath, os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
	if err != nil {
		return fmt.Errorf("wal: failed to create new WAL file: %w", err)
	}
	w.file = file
	return nil
}
